# test-get-stats.ps1 — open \\.\Vigil and read VIGIL_STATS via IOCTL_VIGIL_GET_STATS.
#
...
public struct VIGIL_STATS {
    ...
    public ulong ProcessBlockHits;
    public ulong FileBlockHits;
    public uint  ProcessBlockEntries;
    public uint  FileBlockEntries;
}
...
function Get-VigilStats {
    ...
    $h = [Edr.Native]::CreateFileW("\\.\Vigil", $GENERIC_READ, $FILE_SHARE_READ_WRITE, [IntPtr]::Zero, $OPEN_EXISTING, 0, [IntPtr]::Zero)
    if ($h -eq [IntPtr]::new(-1)) {
        throw "CreateFile \\.\Vigil failed: $([System.Runtime.InteropServices.Marshal]::GetLastWin32Error())"
    }
    try {
        $size = [System.Runtime.InteropServices.Marshal]::SizeOf([type][Edr.Native+VIGIL_STATS])
        $buf = [System.Runtime.InteropServices.Marshal]::AllocHGlobal($size)
        try {
            $bytesReturned = 0
            $ok = [Edr.Native]::DeviceIoControl($h, $IOCTL_VIGIL_GET_STATS, [IntPtr]::Zero, 0, $buf, $size, [ref]$bytesReturned, [IntPtr]::Zero)
            if (-not $ok) {
                throw "DeviceIoControl failed: $([System.Runtime.InteropServices.Marshal]::GetLastWin32Error())"
            }
            return [System.Runtime.InteropServices.Marshal]::PtrToStructure($buf, [type][Edr.Native+VIGIL_STATS])
        } finally {
...
function Format-Stats($s) {
    @(
        ...
        "block=p:$($s.ProcessBlockHits)/$($s.ProcessBlockEntries) f:$($s.FileBlockHits)/$($s.FileBlockEntries)"
...
if ($Watch) {
    while ($true) {
        Write-Host (Format-Stats (Get-VigilStats))
