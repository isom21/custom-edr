# uninstall-edr.ps1 - reverses install-edr.ps1.
#
# Run elevated. Stops + unregisters the agent task and the driver service,
# removes %ProgramFiles%\EDR\, and (with -Purge) the state at
# %ProgramData%\EDR\. The test cert is left in the trust stores by default;
# pass -RemoveCert to clear it too.

[CmdletBinding()]
param(
    [switch]$Purge,        # also remove %ProgramData%\EDR (state, identity, spool)
...
function Assert-Admin {
    ...
    if (-not $principal.IsInRole([Security.Principal.WindowsBuiltinRole]::Administrator)) {
        throw "uninstall-edr.ps1 must be run as Administrator."
    }
}
...
if ($proc) {
    ...
    for ($i = 0; $i -lt 10 -and (Get-Process edr-agent -EA SilentlyContinue); $i++) {
        Start-Sleep -Milliseconds 500
    }
    if (Get-Process edr-agent -EA SilentlyContinue) {
