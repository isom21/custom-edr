...
Creates a separate PG role `edr_audit_writer` and revokes UPDATE /
DELETE / TRUNCATE on `audit_log` from the manager's DB user. The
manager keeps the INSERT grant via membership in `edr_audit_writer`
...
down_revision: Union[str, None] = "9c1d3e7a6b22"
...
def upgrade() -> None:
    ...
    # EDR_DATABASE_URL env var, and the operator runs alembic with a
    ...
    op.execute(
        """
        DO $$ BEGIN
            IF NOT EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'edr_audit_writer') THEN
                CREATE ROLE edr_audit_writer NOLOGIN;
            END IF;
...
def downgrade() -> None:
    ...
    op.execute(
        """
        DO $$ BEGIN
            DROP ROLE IF EXISTS edr_audit_writer;
        EXCEPTION WHEN insufficient_privilege THEN
