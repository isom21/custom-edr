...
// changes, so `cargo build` always carries a fresh vigil.bpf.o.
//
...
fn main() {
    ...
    // Rust). When skipped we still need an `vigil.bpf.o` for the
    ...
    let object_path = ebpf_dir.join("vigil.bpf.o");
    if env::var_os("VIGIL_SKIP_EBPF_BUILD").is_some() {
        println!("cargo:warning=VIGIL_SKIP_EBPF_BUILD set; not invoking ebpf/build.sh");
    ...
    match status {
        ...
        Ok(s) => {
            ...
            println!(
                "cargo:warning=ebpf/build.sh exited {s}; writing stub vigil.bpf.o (build-time only)"
            );
            ensure_stub_object(&object_path);
        }
        Err(e) => {
            println!("cargo:warning=ebpf/build.sh: failed to spawn: {e}; writing stub vigil.bpf.o");
            ensure_stub_object(&object_path);
        }
    }
}
...
fn ensure_stub_object(path: &std::path::Path) {
    ...
    if let Err(e) = std::fs::write(path, stub) {
        panic!(
            "failed to write stub vigil.bpf.o at {}: {e}",
            path.display()
        );
    }
}
